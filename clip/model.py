...
class SelfAttention(nn.Module):
    """
    Self-attention module for processing sequence inputs through multi-head attention.
    
    This module applies self-attention followed by a feed-forward network with layer
    normalization, and extracts context vectors for cross-modal interaction.
    """
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None,
        ...
        self.compound_prompt_nctx = design_details.get('cppa_length', 2)  # Updated from 'cmpa_length'
        self.text_layer = text_layer

    def forward(self, x):
        """
        Perform self-attention and MLP operations.
        
        Args:
            x (torch.Tensor): Input tensor
        
        Returns:
            Tuple[torch.Tensor, torch.Tensor]: Processed tensor and context tensor
        """
        x = x + self.attention(self.ln_1(x))
        x = x + self.mlp(self.ln_2(x))
        
        # Extract context based on layer type
        if self.text_layer:
    ...
    def attention(self, x: torch.Tensor):
        ...
        Args:
            x (torch.Tensor): Input tensor for attention
...
class ResidualAttentionBlock_CPPA(nn.Module):
    """
    Residual attention block for cross-modal prompt-guided prototype alignment.
    
    This block manages interactions between visual and textual features through
    self-attention mechanisms and cross-modal attention, enabling effective 
    alignment of features across modalities.
    
    Args:
        d_model_visual (int): Dimension of visual features
        n_head_visual (int): Number of attention heads for visual pathway
        d_model_text (int): Dimension of text features 
        n_head_text (int): Number of attention heads for text pathway
        attn_mask_visual (torch.Tensor, optional): Attention mask for visual inputs
        attn_mask_text (torch.Tensor, optional): Attention mask for text inputs
        prompt_attn (nn.Module, optional): Prompt attention module (for parameter sharing)
        design_details (dict): Configuration parameters for the model
        i (int): Layer index within the transformer stack
    """
    def __init__(self, d_model_visual: int, n_head_visual: int, d_model_text: int, n_head_text: int, prompt_attn = None, attn_mask_visual: torch.Tensor = None,
        ...
        self.compound_prompt_nctx = design_details['cppa_length']
    ...
    def forward(self, inputs):
        ...
        Args:
            inputs (list): Input tensors and parameters
                - x_visual (torch.Tensor): Visual input tensor
                - x_text (torch.Tensor): Textual input tensor
                - compound_prompts_depth (int): Depth of compound prompts
                - counter (int): Current layer counter
        
        Returns:
            list: Processed tensors and updated parameters
        """
        ...
        if not self.first_layer:
            if compound_prompts_depth > 0:
                ...
                if not (counter > compound_prompts_depth - 1):
                    ...
                    # visual layer
                    # Remove the outputs produced by learnable tokens of previous layer
                    prefix = x_visual[0:x_visual.shape[0] - self.compound_prompt_nctx, :, :] 
                    # Add the learnable tokens of this layer with the input, by replacing previous
                    # layer learnable tokens
                    x_visual = torch.cat([prefix, visual_context], dim=0)
                    ...
                    # First remove the learnable tokens from previous layer
                    prefix = x_text[:1, :, :] 
                    suffix = x_text[1 + self.compound_prompt_nctx:, :, :] 
                    # Add the learnable tokens of this layer with the input, replaced by previous
                    # layer learnable tokens
...
class Transformer(nn.Module):
    """
    Transformer implementation that supports different attention block variants.
    
    This transformer can be configured to use various types of attention blocks
    based on the trainer configuration, including standard attention, visual
    prompting, and cross-modal prompt-guided attention.
    """
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        # Retrieve the current trainer type from design details
        current_trainer = design_details.get('trainer', 'CoOp')
        
        # Select appropriate encoder blocks based on the trainer type
        if current_trainer in ['IVLP', 'VPT']:
        ...
        elif current_trainer == 'CPPA':
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_CPPA(
                    width, heads, attn_mask, 
                    design_details=design_details, 
                    text_layer=text_layer, 
                    i=i
            ...
            # Default to standard ResidualAttentionBlock for CoOp or CoCoOp
            self.resblocks = nn.Sequential(
    ...
    def forward(self, x: torch.Tensor):
        ...
        Args:
            x (torch.Tensor): Input tensor to be processed through transformer blocks
        
        Returns:
            torch.Tensor: Processed output tensor after passing through all transformer blocks
        """
        return self.resblocks(x)

class CPPA(nn.Module):
    """
    Cross-Modal Prompt-guided Prototype Alignment (CPPA) model.
    
    This model implements a dual-stream architecture for vision-language learning,
    with cross-modal interactions facilitated through prompt-guided attention mechanisms.
    """
    def __init__(self,
                 embed_dim: int,
                 # vision
        ...
        self.parameters_sharing = design_details.get('parameter_sharing', True)
        ...
        if self.parameters_sharing:
            ...
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_CPPA(vision_width, vision_heads, transformer_width, transformer_heads, prompt_attn,
                                            attn_mask_visual=None, attn_mask_text=attn_mask_text, design_details=design_details, i=i)
                    for i in range(vision_layers)])
        else:
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_CPPA(vision_width, vision_heads, transformer_width, transformer_heads, prompt_attn=None,
                                            attn_mask_visual=None, attn_mask_text=attn_mask_text, design_details=design_details, i=i)
                    for i in range(vision_layers)])
    ...
    def initialize_parameters(self):
        """
        Initialize model parameters with specific initialization strategies.
        """
    ...
    def build_attention_mask(self):
        """
        Create a causal attention mask with full attention between vision tokens.
        
        Returns:
            torch.Tensor: Attention mask with lower diagonal filled with -inf
        """
    ...
    def dtype(self):
        """
        Get the data type of the visual convolution layer.
        
        Returns:
            torch.dtype: Data type of the visual convolution layer
        """
    ...
    def forward(self, image, text):
        """
        Forward pass of the CPPA model.
        ...
        Returns:
            Tuple[torch.Tensor, torch.Tensor]: Logits for image and text
        """
...
def build_model(state_dict: dict, design_details):
    """
    Build CPPA model from the state dictionary.
    
    Args:
        state_dict (dict): State dictionary containing model weights
        design_details (dict): Design configuration details
    
    Returns:
        nn.Module: Initialized and evaluated CPPA model
    """
    ...
    # Ensure design_details has correct defaults
    if 'parameter_sharing' not in design_details:
        design_details['parameter_sharing'] = True
