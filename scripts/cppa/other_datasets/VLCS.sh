...
source_domains=("Caltech101" "LabelMe" "SUN09" "VOC2007")
...
for ((i=0; i<${#source_domains[@]}; i++)); do
    for ((j=0; j<${#source_domains[@]}; j++)); do
        if [ $i -ne $j ]; then
            ...
            output_dir="./output/domain_adaptation/vlcs/${source_domain}_${target_domain}"
            dataset_config_file="./configs/datasets/VLCS_CPPA.yaml"
            ...
            # Important note about source and target domain parameters
