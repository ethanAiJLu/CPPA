...
# DIGIT5 domains
source_domains=("svhn" "usps" "mnist")

# Define specific migration directions
migrations=(
    "svhn mnist"
    "usps mnist"
    "mnist usps"
...
for migration in "${migrations[@]}"; do
    read -r source_domain target_domain <<< "$migration"

    echo "Now the source domain is $source_domain, and the target domain is $target_domain. Begin to train."

    # Update paths for the Digit5 dataset
    output_dir="./output/digit5/${source_domain}_${target_domain}"
    dataset_config_file="./configs/datasets/DG5_CPPA.yaml"
